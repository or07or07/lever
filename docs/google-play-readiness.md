...
- **Status: PARTIALLY RESOLVED — rate limiter only.** `rate_limiter.py` now supports an optional Redis-backed store, selected once at import time from a new `REDIS_URL` setting (`config.py`, `.env.template`). When unset (the default), behavior is byte-for-byte the same in-memory implementation as before — nothing changes for local dev or a single-instance deployment. When set, the identical sliding-window algorithm runs against Redis instead, using a sorted set per key with `ZREMRANGEBYSCORE`/`ZCARD`/`ZADD` wrapped in a Lua script (`EVAL`) for atomicity — without that, concurrent requests against the same key could each read the count before any of them recorded their own entry, letting more requests through than the configured limit. If `REDIS_URL` is set but Redis is unreachable at startup, it logs a warning and falls back to in-memory rather than crashing the app over a rate-limiter dependency. No real Redis server was available in this sandbox (network access for downloading one wasn't reliable — same issue that blocked the Gradle build test in GP-03/04), so I verified the actual Lua script against `fakeredis` (with real Lua execution via `lupa`, not fakeredis's default no-EVAL mode) instead of just reading the code and trusting it: confirmed it allows exactly N requests and blocks the N+1th, resets correctly once the window passes, and — the case that actually justifies the Lua script over a plain get-then-set — stays exactly at the configured limit (10 allowed out of 30 truly concurrent requests via a thread pool, not just sequential calls) rather than over-admitting under concurrent load. Also booted the real app end-to-end and confirmed the existing login-endpoint rate limit (10 attempts/15min) still enforces identically through the full FastAPI middleware stack, not just in isolation.
  **Not done:** `dispatch.py`'s in-flight offer-rotation timers are still process-local memory, exactly as originally flagged — this pass only touched the rate limiter half of this finding. Moving dispatch state to Redis (or a DB-backed queue) is a separate, standalone task if you want it addressed before scaling past one process.
- **Severity:** Medium
...
- **Status: RESOLVED** — added TOTP-based MFA gated to the `admin` role only (migration `0005`: `mfa_enabled`, `mfa_secret`, `mfa_backup_codes` on `User`; `pyotp` added to `requirements.txt`). Enrollment is a two-step confirm flow (`POST /mfa/setup` generates a secret + 8 single-use backup codes but leaves `mfa_enabled=False`; `POST /mfa/confirm` only flips it on once the admin proves their authenticator app actually produces a matching code) so nobody can lock themselves out via a botched setup. Login for an MFA-enabled admin now returns `mfa_required: true` with a short-lived (5 min) `mfa_token` instead of a real access token; `POST /mfa/verify-login` exchanges that plus a TOTP or backup code for the real token. Backup codes are single-use and stored bcrypt-hashed, never in plaintext after the one-time setup response. `POST /mfa/disable` requires re-entering the password. Surfaced in Settings under a new "Autenticación de Dos Factores" card (admin-only) — the secret is shown as manually-enterable text rather than a QR code, since generating one would mean either adding a new client-side dependency or writing a QR encoder from scratch, and manual key entry is a fully standard, universally-supported alternative in every authenticator app.
  **Found and fixed a real vulnerability during testing:** `get_current_user` only ever checked the `sub` and `ver` claims — it never verified a token actually came from the normal login path. The short-lived `mfa_pending` token (deliberately claim-shaped differently, with `purpose: "mfa_pending"` instead of `role`) would still pass every one of those checks as long as the target admin's `token_version` was still its default `0` (true for essentially every admin who hasn't yet used "logout everywhere" or a password reset) — meaning it could be used directly as a full access token, completely skipping the TOTP step. Fixed by requiring the `role` claim to be present in `get_current_user`, which only real access tokens ever carry. Verified live before and after: pre-fix, `GET /api/auth/me` with a raw `mfa_pending` token returned `200`; post-fix it correctly returns `401`, with no regression to normal login, MFA-completed login, or non-admin tokens.
  **Also found and fixed:** the admin role's frontend router never had a `#/settings` case at all — the entire Settings screen (including the "logout everywhere" button from GP-13 and this MFA card) was unreachable for admins, with no sidebar or bottom-tab link to it either. Added the route and nav entries for both desktop and mobile.
  Verified live end-to-end: full setup → confirm → backup-codes-once flow; wrong TOTP code rejected; correct TOTP code accepted; backup code accepted and consumed (single-use, second attempt with the same code correctly rejected); disable with wrong password rejected, correct password succeeds and returns login to the normal single-step flow; the whole thing exercised through the actual browser UI, not just the API.
- **Severity:** Medium

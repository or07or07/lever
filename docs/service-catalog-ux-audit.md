...
| UX-08 | Provider locked to ONE profession, can't select individual services, no min price, no emergency opt-in, no pause | **RESOLVED (partial)** | Yes (provider steps 2–4) | `mechanic_profiles`, profile UI | ✅ Service selection/pricing/pause + verification gate shipped (migration `0008`, `routes/provider.py`, "Mis Servicios"). Still open: emergency opt-in toggle, min-price field, working-hours/availability (tracked separately, still Phase 3) |
...
| **D5** | Provider verification for high-risk services: what does "enhanced verification" actually mean operationally (cédula photo + admin review? references?) and who reviews it? | Cédula + selfie upload reviewed in the existing admin panel; gate high-risk services on approval | 🟡 **Partial**: `User.verification_level` (`none`/`enhanced`) now exists and is enforced — a provider genuinely cannot select an enhanced-tier service (electrical, construction, home_security core work) without it, and an admin can grant/revoke it from Usuarios today. Still open: no in-app document *upload* — verification currently happens by the provider emailing/WhatsApping their cédula to support, which doesn't scale past a handful of providers. Self-serve upload is D3-adjacent (needs the same media storage decision) and is the next real step here. |
